...
public record UserController(UserService userService) {
	...
	public ResponseEntity<UserDto> findById(
			@Parameter(description = "ID of the user to be found", required = true) @PathVariable("id") Long id) {
		var user = userService.findById(id);
		return ResponseEntity.ok(new UserDto(user));
	}
	
	@GetMapping
	@Operation(summary = "Get all users", description = "Retrieve a list of all resgistered users")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Operation successful")
	})
	...
	public ResponseEntity<UserDto> update(
			@Parameter(description = "ID of the user to be found", required = true) @PathVariable("id") Long id, @RequestBody UserDto userDto) {
		var user = userService.update(id, userDto.toModel());
		return ResponseEntity.ok(new UserDto(user));
	}
	
	@DeleteMapping("/{id}")
	@Operation(summary = "Delete a user", description = "Delete an existing user bades on its ID")
	@ApiResponses(value = {
